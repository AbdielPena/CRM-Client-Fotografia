-- FIX: una factura NO debe marcarse "vencida" antes del día de la sesión, porque
-- el saldo (2do pago) se paga el día de la sesión. run_studio_automations marcaba
-- overdue por due_date < hoy sin mirar la fecha de la sesión. Ahora solo marca
-- vencida si la sesión ya pasó (o no hay fecha de sesión).
-- (Hotfix de datos aplicado en vivo: facturas con sesión futura des-vencidas a
--  partially_paid y due_date alineado = event_date.)
...
declare
  ...
  v_n integer;
begin
  -- 0) Marcar facturas vencidas (NO antes del día de la sesión)
