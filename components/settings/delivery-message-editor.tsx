...
import { updateDeliveryWaMessageAction } from "@/server/actions/share-message.actions"
import {
  DEFAULT_DELIVERY_WA_MESSAGE,
  renderWaMessage,
} from "@/lib/share/wa-message"

export function DeliveryMessageEditor({ initial }: { initial: string }) {
  const [text, setText] = React.useState(initial)
  const [pending, startTransition] = useTransition()

  // El fallback interno de renderWaMessage es el de SELECCIÓN; forzamos el de
  // entrega cuando el textarea está vacío para que la vista previa sea correcta.
  const preview = renderWaMessage(text || DEFAULT_DELIVERY_WA_MESSAGE, {
    cliente: "María",
    galeria: "Quinceañera de María",
    link_web: "https://my.abbypixel.com/g/abc123?entrega=1",
    link_drive: "https://drive.google.com/drive/folders/xxxx",
  })
  ...
  const save = () =>
    startTransition(async () => {
      try {
        await updateDeliveryWaMessageAction(text)
        toast.success("Mensaje de entrega guardado — se usará en todas las galerías")
      } catch {
    ...
    <div className="mt-6 rounded-xl border border-border bg-card p-5">
      <div className="mb-1 flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold text-foreground">
          Mensaje de WhatsApp — Entrega final
        </h2>
      </div>
      <p className="mb-3 text-[12px] text-muted-foreground">
        Mensaje que se arma al compartir la <strong>entrega final</strong> (fotos ya
        editadas) desde la pestaña Compartir. Incluye el link de descarga desde la web
        y el de Google Drive. Lo editás acá una vez y se usa en todas las galerías.
      </p>

      <div className="mb-2 flex flex-wrap items-center gap-1.5 text-[11px]">
        <span className="text-muted-foreground">Variables:</span>
        {["{{cliente}}", "{{galeria}}", "{{link_web}}", "{{link_drive}}"].map((v) => (
          <button
            key={v}
            type="button"
            onClick={() => insertVar(v)}
      ...
      <textarea
        ...
        placeholder={DEFAULT_DELIVERY_WA_MESSAGE}
      />
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button
          onClick={() => setText(DEFAULT_DELIVERY_WA_MESSAGE)}
          disabled={pending}
