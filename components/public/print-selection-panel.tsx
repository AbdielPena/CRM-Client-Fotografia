...
export function PrintSelectionPanel({
  ...
  const [activeKey, setActiveKey] = useState<string>(
    initialState.categories[0]?.key ?? "",
  )
  const [busyAsset, setBusyAsset] = useState<string | null>(null)
  const [pending, startTransition] = useTransition()

  const activeCat = state.categories.find((c) => c.key === activeKey) ?? state.categories[0]
  ...
  const allComplete = state.categories.every((c) => c.used >= c.allowed)
    ...
    <section className="border-t border-gold-200/60 bg-gold-50/40 dark:border-gold-500/20 dark:bg-gold-500/5">
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">
          Elige tus fotos para cada entregable incluido en tu plan. Toca una
          categoría y luego las fotos.
        </p>
