...
export default async function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const quinceName = (project.quinceanera_name as string | null) ?? null
    ...
    <>
      ...
      {(missingTime || missingQuinceName || missingCollaborators || missingDress || missingBirthday) && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {missingQuinceName && (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11.5px] font-medium text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
              <Crown className="h-3.5 w-3.5" /> Falta el nombre de la quinceañera
            </span>
          )}
          {missingBirthday && (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11.5px] font-medium text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="sf-card">
            ...
            {/* Datos de la quinceañera → el nombre nombra sus galerías; el
                cumpleaños define la entrega pautada (2 días antes) */}
            {isQuince && (
              <div className="space-y-1.5 border-t border-border/60 px-5 py-3">
                <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  {quinceName ? (
                    <>
                      Quinceañera:{" "}
                      <span className="font-medium text-foreground">{quinceName}</span>
                      <span className="text-muted-foreground/70">
                        · nombra sus galerías
                      </span>
                    </>
                  ) : (
                    "Nombre de la quinceañera sin registrar — nombra sus galerías"
                  )}
                </p>
                <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
