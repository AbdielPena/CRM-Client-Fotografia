...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Creando tu estudio…" : "Crear mi estudio"}
    </Button>
  )
}
...
export default function RegisterPage() {
  ...
  const err = (name: string) =>
    state.status === "error" ? state.fieldErrors?.[name]?.[0] ?? null : null
    ...
    >
      <header className="space-y-2">
        ...
        <h1 className="font-display text-display-lg leading-[1.05] text-foreground">
          Lanza tu <span className="text-gradient-aurora">estudio</span> en minutos
        </h1>
